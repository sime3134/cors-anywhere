...
function parseURL(req_url) {
  ...
  if (/^https?:\/[^/]/i.test(req_url)) {
    // Add an extra slash after https:
    // "https:/example.com" -> "https://example.com"
    req_url = req_url.replace(/^(https?:)\/([^/])/, '$1//$2');
  }
  ...
  if (!match[1]) {
    if (/^https?:/i.test(req_url)) {
      console.warn('Missing protocol in provided URL:' + req_url);
      // The pattern at top could mistakenly parse "http:///" as host="http:" and path=///.
      return null;
    }
    // Scheme is omitted.
    if (req_url.lastIndexOf('//', 0) === -1) {
      console.warn('Missing // in provided URL, adding one:' + req_url);
      // "//" is omitted.
      req_url = '//' + req_url;
    }
  ...
  if (!parsed.hostname) {
    console.warn('Invalid hostname in provided URL:' + req_url);
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    requireHeader: false,            // Require a header to be set?
